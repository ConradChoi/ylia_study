...
- **규모**: 총 320문제. 시험이 다음 주 토요일이라 내일(금)부터 다음 주 금요일까지 8일간만 순환하면 되므로, 40문제/일 × 8일 = 320문제로 정했다. 실제 SQLD 과목 비율(데이터 모델링의 이해 20% : SQL 기본 및 활용 80%)을 반영해 데이터 모델링 64문제 + SQL 기본/활용 256문제로 구성한다.
- **형식**: 4지선다 객관식. 각 문항은 문제, 보기 4개, 정답, 해설을 포함한다.
- **매일 발행량**: 40문제/일, 8일 주기로 순환 (기존 단어/숙어/구절과 동일한 날짜 기반 결정론적 순환 로직 재사용, 같은 epoch=2026-08-01 공유). 8일이 지나면 처음부터 다시 반복된다 — 시험 이후에도 별도 종료 처리는 하지 않는다(YAGNI).
...
```
sqld_today = pickWindow(dayIndex, 320, 40)  // 40문제/일, 8일 주기
```
